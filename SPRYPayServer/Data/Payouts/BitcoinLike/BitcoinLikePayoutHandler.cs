...
using NewBlockEvent = SPRYPayServer.Events.NewBlockEvent;
...
public class BitcoinLikePayoutHandler : IPayoutHandler
{
    ...
    private readonly SPRYPayNetworkJsonSerializerSettings _jsonSerializerSettings;
    ...
    public BitcoinLikePayoutHandler(SPRYPayNetworkProvider btcPayNetworkProvider,
        WalletRepository walletRepository,
        ExplorerClientProvider explorerClientProvider,
        SPRYPayNetworkJsonSerializerSettings jsonSerializerSettings,
    ...
    public bool CanHandle(PaymentMethodId paymentMethod)
    {
        return paymentMethod?.PaymentType == BitcoinPaymentType.Instance &&
               _btcPayNetworkProvider.GetNetwork<SPRYPayNetwork>(paymentMethod.CryptoCode)?.ReadonlyWallet is false;
    }

    public async Task TrackClaim(PaymentMethodId paymentMethodId, IClaimDestination claimDestination)
    {
        var network = _btcPayNetworkProvider.GetNetwork<SPRYPayNetwork>(paymentMethodId.CryptoCode);
    ...
    {
        if (_btcPayNetworkProvider.GetNetwork<SPRYPayNetwork>(paymentMethodId.CryptoCode)?
                .NBitcoinNetwork?
                .Consensus?
                .ConsensusFactory?
            ...
            claimDestination is IBitcoinLikeClaimDestination bitcoinLikeClaimDestination)
        {
            ...
            return Task.FromResult(txout.GetDustThreshold().ToDecimal(MoneyUnit.SPRY));
        }
        ...
        foreach (var payout in payouts)
        {
            ...
            switch (claim.destination)
            {
                case UriClaimDestination uriClaimDestination:
                    uriClaimDestination.BitcoinUrl.Amount = new Money(blob.CryptoAmount.Value, MoneyUnit.SPRY);
                    var newUri = new UriBuilder(uriClaimDestination.BitcoinUrl.Uri);
                    SPRYPayServerClient.AppendPayloadToQuery(newUri, new KeyValuePair<string, object>("payout", payout.Id));
                    bip21.Add(newUri.Uri.ToString());
                    break;
                case AddressClaimDestination addressClaimDestination:
                    var bip21New = network.GenerateBIP21(addressClaimDestination.Address.ToString(), new Money(blob.CryptoAmount.Value, MoneyUnit.SPRY));
        ...
        AddressTrackedSource addressTrackedSource)
    {
        try
        {
            var network = _btcPayNetworkProvider.GetNetwork<SPRYPayNetwork>(newTransaction.CryptoCode);
            var destinationSum =
...
#pragma warning restore CA1307 // Specify StringComparison
            ...
            if (payoutBlob.CryptoAmount is null ||
                ...
                destinationSum !=
                SPRYPayServer.Extensions.RoundUp(payoutBlob.CryptoAmount.Value, network.Divisibility))
                return;
