...
namespace SPRYPayServer.Data.Payouts.LightningLike
{
    ...
    public class UILightningLikePayoutController : Controller
    {
        ...
        public async Task<IActionResult> ProcessLightningPayout(string cryptoCode, string[] payoutIds, CancellationToken cancellationToken)
        {
            ...
            var network = _btcPayNetworkProvider.GetNetwork<SPRYPayNetwork>(pmi.CryptoCode);
            ...
            LightningLikePayoutHandler handler, PayoutBlob blob, LNURLPayClaimDestinaton lnurlPayClaimDestinaton, Network network, CancellationToken cancellationToken)
        {
            ...
            var lm = new LightMoney(blob.CryptoAmount.Value, LightMoneyUnit.SPRY);
        ...
        {
            var boltAmount = bolt11PaymentRequest.MinimumAmount.ToDecimal(LightMoneyUnit.SPRY);
            ...
            try
            {
                var result = await lightningClient.Pay(bolt11PaymentRequest.ToString(),
                    new PayInvoiceParams()
                    {
                        Amount = bolt11PaymentRequest.MinimumAmount == LightMoney.Zero
                            ? new LightMoney((decimal)payoutBlob.CryptoAmount, LightMoneyUnit.SPRY)
                            : null
                    }, cancellationToken);
                string message = null;
                if (result.Result == PayResult.Ok)
                {
                    message = result.Details?.TotalAmount != null
                        ? $"Paid out {result.Details.TotalAmount.ToDecimal(LightMoneyUnit.SPRY)}"
                        : null;
