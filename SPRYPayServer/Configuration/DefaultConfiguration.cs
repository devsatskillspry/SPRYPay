...
namespace SPRYPayServer.Configuration
{
    public class DefaultConfiguration : StandardConfiguration.DefaultConfiguration
    {
        ...
        {
            var provider = new SPRYPayNetworkProvider(ChainName.Mainnet);
            ...
            app.Option("--rootpath", "The root path in the URL to access SPRYPay (default: /)", CommandOptionType.SingleValue);
            app.Option("--sshconnection", "SSH server to manage SPRYPay under the form user@server:port (default: root@externalhost or empty)", CommandOptionType.SingleValue);
            app.Option("--sshpassword", "SSH password to manage SPRYPay (default: empty)", CommandOptionType.SingleValue);
            app.Option("--sshkeyfile", "SSH private key file to manage SPRYPay (default: empty)", CommandOptionType.SingleValue);
            app.Option("--sshkeyfilepassword", "Password of the SSH keyfile (default: empty)", CommandOptionType.SingleValue);
            app.Option("--sshauthorizedkeys", "Path to a authorized_keys file that SPRYPayServer can modify from the website (default: empty)", CommandOptionType.SingleValue);
            ...
            app.Option("--torservices", "Tor hostnames of available services added to Server Settings (and sets onion header for sprypay). Format: sprypayserver:host.onion:80;btc-p2p:host2.onion:81,SPRY-RPC:host3.onion:82,UNKNOWN:host4.onion:83. (default: empty)", CommandOptionType.SingleValue);
            ...
            foreach (var network in provider.GetAll().OfType<SPRYPayNetwork>())
            {
                ...
                {
                    app.Option($"--{crypto}externallndgrpc", $"The LND gRPC configuration SPRYPay will expose to easily connect to the internal lnd wallet from an external wallet (default: empty)", CommandOptionType.SingleValue);
                    app.Option($"--{crypto}externallndrest", $"The LND REST configuration SPRYPay will expose to easily connect to the internal lnd wallet from an external wallet (default: empty)", CommandOptionType.SingleValue);
                    app.Option($"--{crypto}externalrtl", $"The Ride the Lightning configuration so SPRYPay will expose to easily open it in server settings (default: empty)", CommandOptionType.SingleValue);
        ...
        public override string EnvironmentVariablePrefix => "SPRYPAY_";

        protected override string GetDefaultDataDir(IConfiguration conf)
        {
            return SPRYPayDefaultSettings.GetDefaultSettings(GetNetworkType(conf)).DefaultDataDirectory;
        }

        protected override string GetDefaultConfigurationFile(IConfiguration conf)
        {
            var network = SPRYPayDefaultSettings.GetDefaultSettings(GetNetworkType(conf));
        ...
        protected override string GetDefaultConfigurationFileTemplate(IConfiguration conf)
        {
            ...
            var defaultSettings = SPRYPayDefaultSettings.GetDefaultSettings(networkType);
            ...
            foreach (var n in new SPRYPayNetworkProvider(networkType).GetAll().OfType<SPRYPayNetwork>())
            {
        ...
        {
            return new IPEndPoint(IPAddress.Parse("127.0.0.1"), SPRYPayDefaultSettings.GetDefaultSettings(GetNetworkType(conf)).DefaultPort);
        }
    }
}
