...
namespace SPRYPayServer
{
    ...
    {
        public static DerivationSchemeSettings Parse(string derivationStrategy, SPRYPayNetwork network)
        {
            string error = null;
        ...
        public static bool TryParseFromJson(string config, SPRYPayNetwork network, out DerivationSchemeSettings strategy)
        {
            ArgumentNullException.ThrowIfNull(network);
        ...
        private static bool TryParseXpub(string xpub, DerivationSchemeParser derivationSchemeParser, ref DerivationSchemeSettings derivationSchemeSettings, ref string error, bool electrum = true)
        {
            ...
            try
            {
                ...
                if (derivationSchemeSettings.AccountDerivation is DirectDerivationStrategy direct && !direct.Segwit)
                    derivationSchemeSettings.AccountOriginal = null; // Saving this would be confusing for user, as xpub of electrum is legacy derivation, but for sprypay, it is segwit derivation
                return true;
            }
            catch (Exception exception)
            {
        ...
        public static bool TryParseFromWalletFile(string fileContents, SPRYPayNetwork network, out DerivationSchemeSettings settings, out string error)
        {
            settings = null;
            error = null;
        ...
        public DerivationSchemeSettings(DerivationStrategyBase derivationStrategy, SPRYPayNetwork network)
        {
            ArgumentNullException.ThrowIfNull(network);
            ArgumentNullException.ThrowIfNull(derivationStrategy);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        [JsonIgnore]
        public PaymentMethodId PaymentId => new PaymentMethodId(Network.CryptoCode, PaymentTypes.SPRYLike);
