...
namespace SPRYPayServer.HostedServices
{
    public class BitpayIPNSender : IHostedService
    {
        ...
        async Task Notify(InvoiceEntity invoice, InvoiceEvent invoiceEvent, bool extendedNotification, bool sendMail)
        {
            ...
            // We keep backward compatibility with bitpay by passing SPRY info to the notification
            // we don't pass other info, as it is a bad idea to use IPN data for logic processing (can be faked)
            var btcCryptoInfo = dto.CryptoInfo.FirstOrDefault(c => c.GetpaymentMethodId() == new PaymentMethodId("SPRY", Payments.PaymentTypes.SPRYLike) && !string.IsNullOrEmpty(c.Address));
            if (btcCryptoInfo != null)
            {
#pragma warning disable CS0618
                notification.Data.Rate = dto.Rate;
                notification.Data.Url = dto.Url;
                notification.Data.SPRYDue = dto.SPRYDue;
                notification.Data.SPRYPaid = dto.SPRYPaid;
                notification.Data.SPRYPrice = dto.SPRYPrice;
#pragma warning restore CS0618
                ...
                MailboxAddressValidator.TryParse(e, out MailboxAddress notificationEmail))
            {
                ...
                var storeName = store.StoreName ?? "SPRYPay Server";
