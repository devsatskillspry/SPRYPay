...
namespace SPRYPayServer.Plugins
{
    public static class PluginManager
    {
        public const string SPRYPayPluginSuffix = ".sprypay";
            ...
            IConfiguration config, ILoggerFactory loggerFactory)
        {
            ...
            var plugins = new List<ISPRYPayServerPlugin>();
            ...
            // as the assembly. Except for the system assembly (sprypayserver assembly) which are fake plugins
            ...
            foreach (var toLoad in pluginsToLoad)
            {
                ...
                if (toLoad.PluginIdentifier == "SPRYPayServer.Plugins.NFC")
                {
        ...
        public static void UsePlugins(this IApplicationBuilder applicationBuilder)
        {
            ...
            foreach (var extension in applicationBuilder.ApplicationServices
                .GetServices<ISPRYPayServerPlugin>())
            {
        ...
        private static IEnumerable<ISPRYPayServerPlugin> GetPluginInstancesFromAssembly(Assembly assembly)
        {
            return assembly.GetTypes().Where(type =>
                typeof(ISPRYPayServerPlugin).IsAssignableFrom(type) && type != typeof(PluginService.AvailablePlugin) &&
                !type.IsAbstract).
                Select(type => (ISPRYPayServerPlugin)Activator.CreateInstance(type, Array.Empty<object>()));
        }
        private static ISPRYPayServerPlugin GetPluginInstanceFromAssembly(string pluginIdentifier, Assembly assembly)
        {
            ...
            bool ignoreOrder = false)
        {
            ...
            switch (command.command)
            {
                ...
                case "install":
                    ExecuteCommand(("enable", command.extension), pluginsFolder, true);
                    var fileName = dirName + SPRYPayPluginSuffix;
