...
namespace SPRYPayServer.Plugins.NFC
{
    ...
    public class NFCController : Controller
    {
        ...
        public async Task<IActionResult> SubmitLNURLWithdrawForInvoice([FromBody] SubmitRequest request)
        {
            ...
            if (!methods.TryGetValue(new PaymentMethodId("SPRY", PaymentTypes.LNURLPay), out var lnurlPaymentMethod) &&
                !methods.TryGetValue(new PaymentMethodId("SPRY", PaymentTypes.LightningLike), out lnPaymentMethod))
            {
            ...
            if (lnurlPaymentMethod is not null)
            {
                ...
                var response = await _uilnurlController.GetLNURLForInvoice(request.InvoiceId, "SPRY",
                    due.Satoshi);
