...
using InvoiceData = SPRYPayServer.Data.InvoiceData;
...
namespace SPRYPayServer.Services.Invoices
{
    public class InvoiceRepository
    {
        ...
        public InvoiceRepository(ApplicationDbContextFactory contextFactory,
            SPRYPayNetworkProvider networks, EventAggregator eventAggregator, Logs logs)
        {
            Logs = logs;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private string GetDestination(PaymentMethod paymentMethod)
        {
            ...
            if (paymentMethod.GetId().PaymentType == Payments.PaymentTypes.SPRYLike)
            {
                var network = (SPRYPayNetwork)paymentMethod.Network;
                var details =
        ...
        public async Task<bool> NewPaymentDetails(string invoiceId, IPaymentMethodDetails paymentMethodDetails, SPRYPayNetworkBase network)
        {
            await using var context = _applicationDbContextFactory.CreateContext();
...
#pragma warning restore CA1310 // Specify StringComparison
        ...
        public static byte[] ToBytes<T>(T obj, SPRYPayNetworkBase network = null)
        {
            return ZipUtils.Zip(ToJsonString(obj, network));
        }

        public static T FromBytes<T>(byte[] blob, SPRYPayNetworkBase network = null)
        {
            return network == null
        ...
        public static string ToJsonString<T>(T data, SPRYPayNetworkBase network)
        {
