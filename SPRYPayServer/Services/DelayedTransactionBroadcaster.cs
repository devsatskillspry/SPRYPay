...
namespace SPRYPayServer.Services
{
    public class DelayedTransactionBroadcaster
    {
        ...
        public async Task Schedule(DateTimeOffset broadcastTime, Transaction transaction, SPRYPayNetwork network)
        {
        ...
        private Record ToRecord(PlannedTransaction plannedTransaction)
        {
            ...
            var network = _networkProvider.GetNetwork(s[0]) as SPRYPayNetwork;
            if (network is null)
                return null;
            return new Record()
            {
