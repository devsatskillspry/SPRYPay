...
namespace SPRYPayServer.Services.Wallets
{
    ...
    public class NetworkCoins
    {
        ...
        public SPRYPayWallet Wallet { get; set; }
    }
    public class SPRYPayWallet
    {
        ...
        public SPRYPayWallet(ExplorerClient client, IMemoryCache memoryCache, SPRYPayNetwork network,
        ...
        public async Task<IList<TransactionHistoryLine>> FetchTransactionHistory(DerivationStrategyBase derivationStrategyBase, int? skip = null, int? count = null, TimeSpan? interval = null)
        {
            ...
            else
            {
                ...
                var rows = await ctx.QueryAsync<(string tx_id, DateTimeOffset seen_at, string blk_id, long? blk_height, long balance_change, string asset_id, long confs)>(
                    "SELECT r.tx_id, r.seen_at, t.blk_id, t.blk_height, r.balance_change, r.asset_id, COALESCE((SELECT height FROM get_tip('SPRY')) - t.blk_height + 1, 0) AS confs " +
