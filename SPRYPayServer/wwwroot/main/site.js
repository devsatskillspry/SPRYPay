...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (stickyHeader) {
        document.documentElement.style.scrollPaddingTop = `calc(${stickyHeader.offsetHeight}px + var(--sprypay-space-m))`;
    }
    ...
    // Theme Switch
    delegate('click', '.sprypay-theme-switch', e => {
        e.preventDefault()
        ...
        setColorMode(mode)
        e.target.closest('.sprypay-theme-switch').blur()
    })
    ...
    if (mainNav) {
        const COLLAPSED_KEY = 'sprypay-nav-collapsed'
