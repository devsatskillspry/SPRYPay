...
(function () {
    ...
    var scriptSrcRegex = /\/modal\/sprypay\.js(\?v=.*)?$/;
    ...
    if (window.sprypay) {
        warn('sprypay.js attempted to initialize more than once.');
        return;
    }

    var iframe = document.createElement('iframe');
    iframe.name = 'sprypay';
    iframe.class = 'sprypay';
    ...
    // Removed, see https://github.com/sprypayserver/sprypayserver/issues/2139#issuecomment-768223263
    // iframe.setAttribute('allowtransparency', 'true');
    
    // https://web.dev/async-clipboard/#permissions-policy-integration
    iframe.setAttribute('allow', 'clipboard-read; clipboard-write')

    var origin = 'http://chat.sprypayserver.org join us there, and initialize this with your origin url through setApiUrlPrefix';
    var scriptMatch = thisScript.match(scriptSrcRegex)
    if (scriptMatch) {
        // We can't just take the domain as sprypay can run under a sub path with RootPath
    ...
    function showFrame() {
        if (window.document.getElementsByName('sprypay').length === 0) {
    ...
    function setButtonListeners() {
        var buttons = window.document.querySelectorAll('[data-sprypay-button]');
