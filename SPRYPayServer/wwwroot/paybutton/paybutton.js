...
function inputChanges(event, buttonSize) {
    ...
    var html =
        ...
        // Form
        '<form method="POST" action="' + esc(srvModel.urlRoot) + actionUrl + '" class="sprypay-form sprypay-form--' + (srvModel.fitButtonInline ? 'inline' : 'block') +'">\n' +
    ...
    // Custom amount
        ...
        html += '  <div class="sprypay-custom-container">\n    <div class="sprypay-custom">\n';
    ...
    html += srvModel.payButtonText
        ? `<button type="submit" class="submit" name="submit" style="min-width:${width};min-height:${height};border-radius:4px;border-style:none;background-color:#0f3b21;cursor:pointer;" title="Pay with SPRYPay Server, a Self-Hosted Bitcoin Payment Processor"><span style="color:#fff">${esc(srvModel.payButtonText)}</span>\n` +
          ...
          '</button>'
        : `  <input type="image" class="submit" name="submit" src="${esc(srvModel.payButtonImageUrl)}" style="width:${width}" alt="Pay with SPRYPay Server, a Self-Hosted Bitcoin Payment Processor">\n`;
    html += '</form>';
...
function addInputPrice(name, price, widthInput, min = 0, max = 'none', step = 'any') {
    if (!price) price = min
    return `      <input class="sprypay-input-price" type="number" name="${esc(name)}" min="${min}" max="${max}" step="${step}" value="${price}" data-price="${price}" style="width:${widthInput};" />\n`;
}

function addSlider(price, min, max, step, width) {
    if (!price) price = min
    return `    <input type="range" class="sprypay-input-range" min="${min}" max="${max}" step="${step}" value="${price}" style="width:${width};margin-bottom:15px;" />\n`;
}
...
function addSelectCurrency(currency) {
    ...
    const defaultCurrencies = ['USD', 'GBP', 'EUR', 'SPRY'];
