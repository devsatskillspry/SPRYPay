...
namespace SPRYPayServer.Payments.Bitcoin
{
    ...
    public class NBXplorerListener : IHostedService
    {
        ...
        readonly SPRYPayWalletProvider _Wallets;
        public NBXplorerListener(ExplorerClientProvider explorerClients,
                                SPRYPayWalletProvider wallets,
        ...
        private async Task Listen(SPRYPayWallet wallet)
        {
        ...
        async Task UpdatePaymentStates(SPRYPayWallet wallet)
        {
        ...
        async Task<InvoiceEntity> UpdatePaymentStates(SPRYPayWallet wallet, string invoiceId)
        {
        ...
        async Task<InvoiceEntity> UpdatePaymentStates(SPRYPayWallet wallet, InvoiceEntity invoice)
        {
            ...
            {
                if (payment.GetPaymentMethodId()?.PaymentType != PaymentTypes.SPRYLike)
                    continue;
        ...
        private async Task<int> FindPaymentViaPolling(SPRYPayWallet wallet, SPRYPayNetwork network)
        {
            int totalPayment = 0;
            ...
            foreach (var i in invoices)
            {
                ...
                if (strategy == null)
                    continue;
                var cryptoId = new PaymentMethodId(network.CryptoCode, PaymentTypes.SPRYLike);
        ...
        private DerivationStrategyBase GetDerivationStrategy(InvoiceEntity invoice, SPRYPayNetworkBase network)
        {
            return invoice.GetSupportedPaymentMethod<DerivationSchemeSettings>(new PaymentMethodId(network.CryptoCode, PaymentTypes.SPRYLike))
                          .Select(d => d.AccountDerivation)
                          .FirstOrDefault();
        }

        private async Task<InvoiceEntity> ReceivedPayment(SPRYPayWallet wallet, InvoiceEntity invoice, PaymentEntity payment, DerivationStrategyBase strategy)
        {
            var paymentData = (BitcoinLikePaymentData)payment.GetCryptoPaymentData();
            invoice = (await UpdatePaymentStates(wallet, invoice.Id));
            if (invoice == null)
                return null;
            var paymentMethod = invoice.GetPaymentMethod(wallet.Network, PaymentTypes.SPRYLike);
