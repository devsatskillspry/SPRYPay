...
using SPRYPayServer.Hosting;
using SPRYPayServer.Logging;
using SPRYPayServer.Models;
using SPRYPayServer.Models.ServerViewModels;
...
using SPRYPayServer.Storage.Services.Providers;
...
{
    [Authorize(Policy = SPRYPayServer.Client.Policies.CanModifyServerSettings,
    ...
    public partial class UIServerController : Controller
    {
        ...
        public IActionResult Maintenance()
        {
            ...
            if (!vm.CanUseSSH)
                TempData[WellKnownTempData.ErrorMessage] = "Maintenance feature requires access to SSH properly configured in SPRYPay Server configuration.";
        ...
        public async Task<IActionResult> Maintenance(MaintenanceViewModel vm, string command)
        {
            ...
            if (command == "changedomain")
            {
                ...
                }))
                {
                    try
                    {
                        ...
                        if (!hasCommonAddress)
                        {
                            ModelState.AddModelError(nameof(vm.DNSDomain), $"Invalid host ({vm.DNSDomain} is not pointing to this SPRYPay instance)");
                            return View(vm);
                        }
                    }
                    catch (Exception ex)
            ...
            {
                var error = await RunSSH(vm, $"sprypay-update.sh");
                if (error != null)
                    return error;
            ...
            {
                var error = await RunSSH(vm, $"sprypay-clean.sh");
                if (error != null)
                    return error;
            ...
            {
                var error = await RunSSH(vm, $"sprypay-restart.sh");
                if (error != null)
                    return error;
                Logs.PayServer.LogInformation("A hard restart has been requested");
                TempData[WellKnownTempData.SuccessMessage] = $"SPRYPay will restart momentarily.";
            }
        ...
        private async Task<IActionResult?> RunSSH(MaintenanceViewModel vm, string command)
        {
            ...
            _ = RunSSHCore(sshClient, $". /etc/profile.d/sprypay-env.sh && nohup {command} > /dev/null 2>&1 & disown");
            return null;
        }
        ...
        public async Task<IActionResult> Policies([FromServices] SPRYPayNetworkProvider btcPayNetworkProvider, PoliciesSettings settings, string command = "")
        {
            ViewBag.UpdateUrlPresent = _Options.UpdateUrl != null;
            ViewBag.AppsList = await GetAppSelectList();
        ...
        private static bool TryParseAsExternalService(TorService torService, [MaybeNullWhen(false)] out ExternalService externalService)
        {
            ...
            if (torService.ServiceType == TorServiceType.RPC)
            {
                externalService = new ExternalService()
                {
                    ...
                    ConnectionString = new ExternalConnectionString(new Uri($"btcrpc://btcrpc:sprypayserver4ever@{torService.OnionHost}:{torService.VirtualPort}?label=SPRYPayNode", UriKind.Absolute)),
                    ServiceName = torService.Name
                };
            }
                ...
                if (service.Type == ExternalServiceTypes.LNDSeedBackup)
                {
                    ...
                    if (!model.IsWalletUnlockPresent)
                    {
                        TempData.SetStatusMessageModel(new StatusMessageModel()
                        {
                            ...
                            "It's recommended, but not required, that you migrate as instructed by <a href=\"https://blog.sprypayserver.org/sprypay-lnd-migration\">our migration blog post</a>.<br />" +
                            "You will need to close all of your channels, and migrate your funds as <a href=\"https://blog.sprypayserver.org/sprypay-lnd-migration\">we documented</a>."
                        });
                    }
        ...
        public async Task<IActionResult> DeleteDynamicDnsService(string hostname)
        {
            ...
            return View("Confirm",
                new ConfirmModel("Delete dynamic DNS service",
                    $"Deleting the dynamic DNS service for <strong>{Html.Encode(hostname)}</strong> means your SPRYPay Server will stop updating the associated DNS record periodically.", "Delete"));
        }
        ...
        {
            return View("Confirm", new ConfirmModel("Disable modification of SSH settings", "This action is permanent and will remove the ability to change the SSH settings via the SPRYPay Server user interface.", "Disable"));
        }
        ...
        public async Task<IActionResult> SSHServiceDisablePost()
        {
            ...
            TempData[WellKnownTempData.SuccessMessage] = "Changes to the SSH settings are now permanently disabled in the SPRYPay Server user interface";
            return RedirectToAction(nameof(Services));
        }
        ...
        public async Task<IActionResult> Emails(EmailsViewModel model, string command)
        {
            if (command == "Test")
            {
                try
                {
                    ...
                    using (var message = model.Settings.CreateMailMessage(MailboxAddress.Parse(model.TestEmail), "SPRYPay test", "SPRYPay test", false))
                    {
