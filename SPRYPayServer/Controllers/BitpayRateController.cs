    ...
    public class BitpayRateController : Controller
    {
        ...
        public async Task<IActionResult> GetRates2(string currencyPairs, string storeId, CancellationToken cancellationToken)
        {
            ...
            if (currencyPairs == null)
            {
                ...
                {
                    var result = Json(new BitpayErrorsModel() { Error = "You need to setup the default currency pairs in 'Store Settings / Rates' or specify 'currencyPairs' query parameter (eg. SPRY_USD,LTC_CAD)." });
