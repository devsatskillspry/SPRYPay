    ...
    public partial class UIInvoiceController : Controller
    {
        ...
        internal async Task<InvoiceEntity> CreateInvoiceCoreRaw(InvoiceEntity entity, StoreData store, IPaymentFilter? invoicePaymentMethodFilter, string[]? additionalSearchTerms = null, CancellationToken cancellationToken = default, Action<InvoiceEntity>? entityManipulator = null)
        {
            ...
            foreach (var network in store.GetSupportedPaymentMethods(_NetworkProvider)
                                                ...
                                                .Select(c => _NetworkProvider.GetNetwork<SPRYPayNetworkBase>(c.PaymentId.CryptoCode))
                                                .Where(c => c != null))
            {
            ...
            if (!noNeedForMethods)
            {
                ...
                var x1 = store.GetSupportedPaymentMethods(_NetworkProvider)
                    ...
                    .Select(c =>
                        (Handler: _paymentMethodHandlerDictionary[c.PaymentId],
                            ...
                            Network: _NetworkProvider.GetNetwork<SPRYPayNetworkBase>(c.PaymentId.CryptoCode)))
                ...
                if (supported.Count == 0)
                {
                    ...
                    if (!store.GetSupportedPaymentMethods(_NetworkProvider).Any())
                        errors.AppendLine(
                            "Warning: No wallet has been linked to your SPRYPay Store. See the following link for more information on how to connect your store and wallet. (https://docs.sprypayserver.org/WalletSetup/)");
                    else
            ...
            using (logs.Measure("Saving invoice"))
            {
                ...
                foreach (var method in paymentMethods)
                {
                    if (method.GetPaymentMethodDetails() is BitcoinLikeOnChainPaymentMethod bp)
                    {
                        ...
                        if (bp.GetDepositAddress(((SPRYPayNetwork)method.Network).NBitcoinNetwork) is BitcoinAddress address)
                        {
        ...
        private async Task<PaymentMethod?> CreatePaymentMethodAsync(
            ...
            IPaymentMethodHandler handler, ISupportedPaymentMethod supportedPaymentMethod, SPRYPayNetworkBase network,
            ...
            HashSet<PaymentMethodId> invoicePaymentMethods)
        {
            try
            {
                ...
                if (criteria?.Value != null && entity.Type != InvoiceType.TopUp)
                {
                    ...
                    {
                        var amount = paymentMethod.Calculate().Due.GetValue(network as SPRYPayNetwork);
...
#pragma warning disable CS0618
                if (paymentMethod.GetId().IsSPRYOnChain)
                {
