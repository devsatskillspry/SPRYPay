...
namespace SPRYPayServer.Controllers
{
    public partial class UIInvoiceController
    {
        public class FakePaymentRequest
        {
            ...
            public string CryptoCode { get; set; } = "SPRY";
            public string PaymentMethodId { get; set; } = "SPRY";
        }
        ...
        public async Task<IActionResult> TestPayment(string invoiceId, FakePaymentRequest request, [FromServices] Cheater cheater)
        {
            ...
            var cryptoCode = isSats ? "SPRY" : request.CryptoCode;
            var amount = new Money(request.Amount, isSats ? MoneyUnit.Satoshi : MoneyUnit.SPRY);
            var network = _NetworkProvider.GetNetwork<SPRYPayNetwork>(cryptoCode).NBitcoinNetwork;
            ...
            try
            {
                ...
                switch (paymentMethod?.GetId().PaymentType)
                {
                    case BitcoinPaymentType:
                        ...
                        {
                            Txid = txid,
                            AmountRemaining = (paymentMethod.Calculate().Due - amount).ToUnit(MoneyUnit.SPRY),
                            SuccessMessage = $"Created transaction {txid}"
                        });

                    case LightningPaymentType:
                        // requires the channels to be set up using the SPRYPayServer.Tests/docker-lightning-channel-setup.sh script
                        LightningConnectionString.TryParse(Environment.GetEnvironmentVariable("SPRYPAY_SPRYEXTERNALLNDREST"), false, out var lnConnection);
                        ...
                        if (response.Result == PayResult.Ok)
                        {
                            ...
                            {
                                Txid = paymentHash,
                                AmountRemaining = (paymentMethod.Calculate().TotalDue - paid).ToUnit(MoneyUnit.SPRY),
