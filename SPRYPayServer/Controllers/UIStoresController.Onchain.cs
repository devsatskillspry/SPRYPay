...
namespace SPRYPayServer.Controllers
{
    public partial class UIStoresController
    {
        ...
        public async Task<IActionResult> UpdateWallet(WalletSetupViewModel vm)
        {
            ...
            PaymentMethodId paymentMethodId = new PaymentMethodId(network.CryptoCode, PaymentTypes.SPRYLike);
        ...
        public async Task<IActionResult> GenerateWallet(string storeId, string cryptoCode, WalletSetupMethod method, WalletSetupRequest request)
        {
            ...
            var vm = new WalletSetupViewModel
            {
                ...
                DerivationSchemeFormat = "SPRYPay",
            ...
            if (needUpdate)
            {
                ...
                if (payjoinChanged && blob.PayJoinEnabled)
                {
                    ...
                    if (problematicPayjoinEnabledMethods.Any())
                    {
                        ...
                        {
                            Severity = StatusMessageModel.StatusSeverity.Warning,
                            Html = $"The payment settings were updated successfully. However, PayJoin will not work for {string.Join(", ", problematicPayjoinEnabledMethods)} until you configure them to be a <a href='https://docs.sprypayserver.org/HotWallet/' class='alert-link' target='_blank'>hot wallet</a>."
                        });
                    }
                }
            }
        ...
        private ActionResult IsAvailable(string cryptoCode, out StoreData store, out SPRYPayNetwork network)
        {
        ...
        {
            var id = new PaymentMethodId(cryptoCode, PaymentTypes.SPRYLike);
