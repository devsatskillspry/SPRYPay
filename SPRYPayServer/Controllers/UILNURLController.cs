...
using MarkPayoutRequest = SPRYPayServer.HostedServices.MarkPayoutRequest;
...
namespace SPRYPayServer
{
    ...
    public class UILNURLController : Controller
    {
        ...
        public async Task<IActionResult> GetLNURLForPullPayment(string cryptoCode, string pullPaymentId, string pr, CancellationToken cancellationToken)
        {
            ...
            {
                MaxWithdrawable = LightMoney.FromUnit(remaining, LightMoneyUnit.SPRY),
                K1 = pullPaymentId,
                BalanceCheck = new Uri(Request.GetCurrentUrl()),
                CurrentBalance = LightMoney.FromUnit(remaining, LightMoneyUnit.SPRY),
                MinWithdrawable =
                    LightMoney.FromUnit(
            ...
            var claimResponse = await _pullPaymentHostedService.Claim(new ClaimRequest()
            {
                ...
                StoreId = pp.StoreId,
                Value = result.MinimumAmount.ToDecimal(LightMoneyUnit.SPRY)
            });
        ...
        public async Task<IActionResult> ResolveLightningAddress(string username)
        {
            ...
            return await GetLNURL("SPRY", lightningAddressSettings.StoreDataId, blob.CurrencyCode, blob.Min, blob.Max,
                ...
                internalDetails = null)
        {
            ...
            return Ok(new LNURLPayRequest
            {
                ...
                MaxSendable =
                    max is null
                        ? LightMoney.FromUnit(6.12m, LightMoneyUnit.SPRY)
            ...
            [FromQuery] long? amount = null, string comment = null)
        {
            ...
            if (i.Status == InvoiceStatusLegacy.New)
            {
                ...
                var max = isTopup ? LightMoney.FromUnit(6.12m, LightMoneyUnit.SPRY) : min;
