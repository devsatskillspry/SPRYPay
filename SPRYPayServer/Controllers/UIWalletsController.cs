...
using SPRYPayServer.ModelBinders;
using SPRYPayServer.Models;
using SPRYPayServer.Models.WalletViewModels;
...
using SPRYPayServer.Services.Wallets.Export;
using Dapper;
...
namespace SPRYPayServer.Controllers
{
    ...
    public partial class UIWalletsController : Controller
    {
        ...
        private SPRYPayNetworkProvider NetworkProvider { get; }
            ...
            [FromQuery] string? returnUrl = null)
        {
            ...
            if (paymentMethod == null)
                return NotFound();
            var network = NetworkProvider.GetNetwork<SPRYPayNetwork>(walletId.CryptoCode);
            if (network == null)
                return NotFound();
            var store = GetCurrentStore();
            ...
            [FromQuery] string? returnUrl = null)
        {
            ...
            if (paymentMethod == null || store is null)
                return NotFound();
            var network = this.NetworkProvider.GetNetwork<SPRYPayNetwork>(walletId.CryptoCode);
        ...
        private async Task<string?> GetSeed(WalletId walletId, SPRYPayNetwork network)
        {
        ...
        private void LoadFromBIP21(WalletSendModel vm, string bip21, SPRYPayNetwork network)
        {
            vm.Outputs ??= new();
            try
            {
                ...
                vm.Outputs.Add(new WalletSendModel.TransactionOutput()
                {
                    Amount = uriBuilder.Amount?.ToDecimal(MoneyUnit.SPRY),
            ...
            SignWithSeedViewModel viewModel)
        {
            ...
            var changed = psbt.PSSPRYhanged(() => psbt.SignAll(settings.AccountDerivation, signingKey, rootedKeyPath));
            if (!changed)
            {
                ...
                changed = psbt is not null && psbt.PSSPRYhanged(() =>
        ...
        private string ValueToString(Money v, SPRYPayNetworkBase network)
        {
        ...
        private static async Task<IMoney> GetBalanceAsMoney(SPRYPayWallet wallet,
        ...
        internal async Task<string> GetBalanceString(SPRYPayWallet wallet, DerivationStrategyBase? derivationStrategy)
        {
            ...
            string format, string? labelFilter = null)
        {
            ...
            {
                FileName = $"sprypay-{walletId}-{DateTime.UtcNow.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture)}.{format}",
                Inline = true
            };
        ...
        private string GetImage(PaymentMethodId paymentMethodId, SPRYPayNetwork network)
        {
            var res = paymentMethodId.PaymentType == PaymentTypes.SPRYLike
