...
namespace SPRYPayServer.Controllers
{
    ...
    public partial class UIStoresController : Controller
    {
        public UIStoresController(
            ...
            SPRYPayServerEnvironment sprypayEnv,
            ...
            IHtmlHelper html)
        {
            ...
            _BtcpayServerOptions = sprypayServerOptions;
            _SPRYPayEnv = sprypayEnv;
            _externalServiceOptions = externalServiceOptions;
            Html = html;
        }

        readonly SPRYPayServerOptions _BtcpayServerOptions;
        readonly SPRYPayServerEnvironment _SPRYPayEnv;
        readonly IServiceProvider _ServiceProvider;
        readonly SPRYPayNetworkProvider _NetworkProvider;
        readonly SPRYPayWalletProvider _WalletProvider;
        ...
        public async Task<IActionResult> Rates(RatesViewModel model, string? command = null, string? storeId = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                ModelState.AddModelError(nameof(model.DefaultCurrencyPairs), "Invalid currency pairs (should be for example: SPRY_USD,SPRY_CAD,SPRY_JPY)");
            }
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            if (command == "Test")
            {
                ...
                {
                    ModelState.AddModelError(nameof(model.ScriptTest), "Fill out currency pair to test for (like SPRY_USD,SPRY_CAD)");
                    return View(model);
                }
                ...
                foreach (var pair in splitted)
                {
                    ...
                    {
                        ModelState.AddModelError(nameof(model.ScriptTest), $"Invalid currency pair '{pair}' (it should be formatted like SPRY_USD,SPRY_CAD)");
                        return View(model);
                    }
        ...
        PaymentMethodOptionViewModel.Format? GetDefaultPaymentMethodChoice(StoreData storeData)
        {
            ...
            {
                defaultChoice = enabled.FirstOrDefault(e => e.CryptoCode == _NetworkProvider.DefaultNetwork.CryptoCode && e.PaymentType == PaymentTypes.SPRYLike) ??
                ...
                if (!string.IsNullOrWhiteSpace(methodCriterion.Value))
                {
                    if (!CurrencyValue.TryParse(methodCriterion.Value, out var value))
                    {
                        model.AddModelError(viewModel => viewModel.PaymentMethodCriteria[index].Value,
                            $"{methodCriterion.PaymentMethod}: Invalid format. Make sure to enter a valid amount and currency code. Examples: '5 USD', '0.001 SPRY'", this);
                    }
                }
            }
            ...
            foreach (var paymentMethodId in _paymentMethodHandlerDictionary.Distinct().SelectMany(handler => handler.GetSupportedPaymentMethods()))
            {
                switch (paymentMethodId.PaymentType)
                {
                    case BitcoinPaymentType _:
                        ...
                        var network = _NetworkProvider.GetNetwork<SPRYPayNetwork>(paymentMethodId.CryptoCode);
...
#if ALTCOINS
                            Collapsed = network is ElementsSPRYPayNetwork elementsSPRYPayNetwork && elementsSPRYPayNetwork.NetworkCryptoCode != elementsSPRYPayNetwork.CryptoCode && string.IsNullOrEmpty(value)
#endif
                        });
                        break;
        ...
        private DerivationSchemeSettings ParseDerivationStrategy(string derivationScheme, SPRYPayNetwork network)
        {
