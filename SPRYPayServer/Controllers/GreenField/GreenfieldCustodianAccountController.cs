...
using SPRYPayServer.Services.Custodian;
...
using CustodianAccountDataClient = SPRYPayServer.Client.Models.CustodianAccountData;
...
namespace SPRYPayServer.Controllers.Greenfield
{
    ...
    public class GreenfieldCustodianAccountController : ControllerBase
    {
        ...
        {
            // TODO add SATS check everywhere. We cannot change to 'SPRY' ourselves because the qty / price would be different too.
            if ("SATS".Equals(request.FromAsset) || "SATS".Equals(request.ToAsset))
            {
                return this.CreateAPIError(400, "use-asset-synonym",
                    $"Please use 'SPRY' instead of 'SATS'.");
            }
