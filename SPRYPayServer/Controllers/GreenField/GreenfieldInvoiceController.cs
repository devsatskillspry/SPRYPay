...
using NBitcoin;
using CreateInvoiceRequest = SPRYPayServer.Client.Models.CreateInvoiceRequest;
...
namespace SPRYPayServer.Controllers.Greenfield
{
    ...
    public class GreenfieldInvoiceController : Controller
    {
        ...
        public GreenfieldInvoiceController(UIInvoiceController invoiceController, InvoiceRepository invoiceRepository,
            LinkGenerator linkGenerator, LanguageService languageService, SPRYPayNetworkProvider btcPayNetworkProvider,
        ...
        public async Task<IActionResult> CreateInvoice(string storeId, CreateInvoiceRequest request)
        {
            ...
            if (request.Checkout.DefaultLanguage != null)
            {
                ...
                if (lang == null)
                {
                    ...
                    "The requested defaultLang does not exists, Browse the ~/misc/lang page of your SPRYPay Server instance to see the list of supported languages.", this);
                }
                else
                {
        ...
        )
        {
            ...
            var cryptoPaid = invoicePaymentMethod.Calculate().Paid.ToDecimal(MoneyUnit.SPRY);
        ...
        private Client.Models.PullPaymentData CreatePullPaymentData(Data.PullPaymentData pp)
        {
            ...
            return new SPRYPayServer.Client.Models.PullPaymentData()
            {
                Id = pp.Id,
            ...
            return entity.GetPaymentMethods().Select(
                method =>
                {
                    ...
                    return new InvoicePaymentMethodDataModel
                    {
                        ...
                        Rate = method.Rate,
                        Due = accounting.DueUncapped.ToDecimal(MoneyUnit.SPRY),
                        TotalPaid = accounting.Paid.ToDecimal(MoneyUnit.SPRY),
                        PaymentMethodPaid = accounting.CryptoPaid.ToDecimal(MoneyUnit.SPRY),
                        Amount = accounting.TotalDue.ToDecimal(MoneyUnit.SPRY),
                        NetworkFee = accounting.NetworkFee.ToDecimal(MoneyUnit.SPRY),
                        PaymentLink =
