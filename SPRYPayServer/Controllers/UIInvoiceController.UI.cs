...
using SPRYPayServer.Models.PaymentRequestViewModels;
...
using SPRYPayServer.Services.Invoices.Export;
...
using BitpayCreateInvoiceRequest = SPRYPayServer.Models.BitpayCreateInvoiceRequest;
...
namespace SPRYPayServer.Controllers
{
    public partial class UIInvoiceController
    {
        ...
        public async Task<IActionResult> Refund(string invoiceId, RefundModel model, CancellationToken cancellationToken)
        {
            ...
            switch (model.RefundStep)
            {
                case RefundSteps.SelectPaymentMethod:
                    ...
                    {
                        var cryptoPaid = paymentMethod.Calculate().Paid.ToDecimal(MoneyUnit.SPRY);
            ...
            var model = new InvoiceDetailsModel
            {
                ...
                CryptoPayments = invoice.GetPaymentMethods().Select(
                    data =>
                    {
                        ...
                        var overpaidAmount = accounting.OverpaidHelper.ToDecimal(MoneyUnit.SPRY);
                        ...
                        return new InvoiceDetailsModel.CryptoPayment
                        {
                            ...
                            PaymentMethod = paymentMethodId.ToPrettyString(),
                            Due = _CurrencyNameTable.DisplayFormatCurrency(accounting.Due.ToDecimal(MoneyUnit.SPRY),
                                paymentMethodId.CryptoCode),
                            Paid = _CurrencyNameTable.DisplayFormatCurrency(
                                accounting.CryptoPaid.ToDecimal(MoneyUnit.SPRY),
                                paymentMethodId.CryptoCode),
            ...
            if (selectedItems != null)
            {
                switch (command)
                {
                    ...
                    case "cpfp":
                        ...
                        if (explorer is null)
                            return NotSupported("This feature is only available to SPRY wallets");
                        ...
                        var bumpableAddresses = (await GetAddresses(selectedItems))
                                                .Where(p => p.GetPaymentMethodId().IsSPRYOnChain)
        ...
        private async Task<PaymentModel?> GetInvoiceModel(string invoiceId, PaymentMethodId? paymentMethodId, string? lang)
        {
            ...
            var storeBlob = store.GetStoreBlob();
            var btcId = PaymentMethodId.Parse("SPRY");
            var lnId = PaymentMethodId.Parse("SPRY_LightningLike");
            var lnurlId = PaymentMethodId.Parse("SPRY_LNURLPAY");
            if (paymentMethodId is null)
            {
                ...
                if (paymentMethodId is null)
                {
                    paymentMethodId = enabledPaymentIds.FirstOrDefault(e => e.CryptoCode == _NetworkProvider.DefaultNetwork.CryptoCode && e.PaymentType == PaymentTypes.SPRYLike) ??
            ...
            SPRYPayNetworkBase network = _NetworkProvider.GetNetwork<SPRYPayNetworkBase>(paymentMethodId.CryptoCode);
            ...
            var model = new PaymentModel
            {
                ...
                HtmlTitle = storeBlob.HtmlTitle ?? "SPRYPay Invoice",
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task<IActionResult> Export(string format, string? storeId = null, string? searchTerm = null, int timezoneOffset = 0)
        {
            ...
            {
                FileName = $"sprypay-export-{DateTime.UtcNow.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture)}.{format}",
                Inline = true
            };
