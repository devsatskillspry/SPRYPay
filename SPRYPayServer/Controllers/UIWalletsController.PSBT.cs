...
using SPRYPayServer.Payments.PayJoin.Sender;
...
namespace SPRYPayServer.Controllers
{
    ...
    {
        [NonAction]
        public async Task<CreatePSBTResponse> CreatePSBT(SPRYPayNetwork network, DerivationSchemeSettings derivationSettings, WalletSendModel sendModel, CancellationToken cancellationToken)
        {
            ...
            WalletPSBTViewModel vm, string command)
        {
            ...
            switch (command)
            {
                ...
                case "combine":
                    ModelState.Remove(nameof(vm.PSBT));
                    return View(nameof(WalletPSSPRYombine), new WalletPSSPRYombineViewModel
                    {
        ...
        private async Task<PSBT> GetPayjoinProposedTX(BitcoinUrlBuilder bip21, PSBT psbt, DerivationSchemeSettings derivationSchemeSettings, SPRYPayNetwork btcPayNetwork, CancellationToken cancellationToken)
        {
        ...
        private async Task FetchTransactionDetails(DerivationSchemeSettings derivationSchemeSettings, WalletPSBTReadyViewModel vm, SPRYPayNetwork network)
        {
        ...
        public async Task<IActionResult> WalletPSSPRYombine([ModelBinder(typeof(WalletIdModelBinder))]
            WalletId walletId, WalletPSSPRYombineViewModel vm)
        {
            var network = NetworkProvider.GetNetwork<SPRYPayNetwork>(walletId.CryptoCode);
            var psbt = await vm.GetPSBT(network.NBitcoinNetwork);
            if (psbt == null)
