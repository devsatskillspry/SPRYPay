...
using SPRYPayServer.Plugins.PointOfSale.Models;
...
using PeterO.Cbor;
using LightningAddressData = SPRYPayServer.Data.LightningAddressData;
using PayoutData = SPRYPayServer.Data.PayoutData;
using PullPaymentData = SPRYPayServer.Data.PullPaymentData;
...
namespace SPRYPayServer.Hosting
{
    public class MigrationStartupTask : IStartupTask
    {
        ...
        private readonly SPRYPayNetworkJsonSerializerSettings _btcPayNetworkJsonSerializerSettings;
        ...
        public MigrationStartupTask(
            ...
            SPRYPayNetworkJsonSerializerSettings btcPayNetworkJsonSerializerSettings,
...
#pragma warning restore CS0612 // Type or member is obsolete
        ...
        {
            var old = await _Settings.GetSettingAsync<UILNURLController.LightningAddressSettings>("SPRYPayServer.LNURLController+LightningAddressSettings");
            if (old is not null)
            {
                ...
                try
                {
                    ...
                    if (db.Database.IsNpgsql())
                    {
                        if (await db.GetMigrationState() == "pending")
                            throw new ConfigException("This database hasn't been completely migrated, please retry migration by setting the SPRYPAY_SQLITEFILE or SPRYPAY_MYSQL setting on top of SPRYPAY_POSTGRES");
                    }
                }
                // Starting up
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public class RateRule_Obsolete
        {
            ...
            public decimal Apply(SPRYPayNetworkBase network, decimal rate)
            {
